...
//            }
    ...
    private byte[] encrypt(byte[] text) {
        ...
        }//lưu giá trị ban đầu vào state[0]

        cipher(state[0], state[1]); // thực hiện mã hóa và lưu và state[1]
        for (int i = 0; i < Nb; i++) {
            for (int j = 0; j < 4; j++) {
                out[i * Nb + j] = (byte) (state[1][j][i] & 0xff);
            }
        }
        return out; // trả về mảng byte đã được mã hóa
    }
    ...
    public String ECB_decrypt(String inputText) {
        byte[] text = hexToByte(inputText);
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        for (int i = 0; i < text.length; i += 16) {
            try {
